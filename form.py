...
class AddBookForm(FlaskForm):
    ...
    npages = IntegerField('Number of Pages',  validators=[InputRequired('Number of pages is required!'), NumberRange(min=1, max=20000, message='Must be between 1 and 20000.')])
    coverimageurl = StringField('Cover Image Url', validators=[URL(message='Must be a valid URL.')])
    bookdescription = TextAreaField('Book Description', validators=[InputRequired('A coverimageurl is required!'), Length(min=3, max=3500, message='Must be between 3 and 3500 characters.')])
    bookreview = TextAreaField('Book Review', validators=[InputRequired('A bookreview is required!'), Length(min=3, max=3500, message='Must be between 3 and 3500 characters.')])