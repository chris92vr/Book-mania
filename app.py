...
def browse(book_id):
    ...
    return render_template("browse.html", book=book,
                           comments=comments, page_title=page_title)
...
def profile():
    ...
    if "user" in session:
        ...
        mycomment = mongo.db.comment.find({"created_by":
                                          session["user"]}).count()
        # count of reviews viewed by the current user
        review_viewed = mongo.db.users.find_one_or_404({'username':
                                                       session["user"]})
        # renders the the page of the user profile
        return render_template("profile.html", username=username,
                               page_title=page_title,
                               myreview=myreview, mycomment=mycomment,
